...
const float darkSquare[3] = { 0.3f, 0.45f, 0.6f };
const float lightSquare[3] = { 0.92f, 0.91f, 0.82f };

const float highlightedDarkSquare[3] = { 0.16f,0.55f,0.80f };
const float highlightedLightSquare[3] = { 0.47f,0.77f,0.93f };

const float captureDarkSquare[3] = { 0.92f, 0.25f, 0.20f };
const float captureLightSquare[3] = { 0.92f, 0.25f, 0.20f };
...
void window_size_callback(GLFWwindow* window, int width, int height) {
    int newDimension = (width) < (height) ? (width) : (height);
...
std::string getBestMove(std::string& result) {
...
void makeStockFishMove(WindowData* windowData) {
    ...
    std::string bestMove = getBestMove(output);
    moves += " " + bestMove;
    std::vector<int> move = stringToCordMove(bestMove);
    if (bestMove[4]!=' ') windowData->chessboard->setPromotionType(bestMove[4]);
...
int main() {
    ...
    StockfishProcess stockfish("C:\\Users\\ninja\\source\\repos\\OpenGLTest\\stockfish\\stockfish-windows-x86-64-avx2.exe");
